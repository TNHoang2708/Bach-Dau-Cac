...
function BMI() {
    ...
    const tinhBMI = () => {
        ...
        if (bmi < 18.5) {
            phanLoai = 'Thiếu cân'
            mauSac = '#60a5fa'
            moTa = 'Bạn đang thiếu cân so với chuẩn.'
            goiY = 'Tăng cường ăn uống, bổ sung protein và calo. Tập các bài tăng cơ như squat, deadlift, bench press.'
        } else if (bmi < 23) {
            phanLoai = 'Bình thường'
            mauSac = '#00d4a0'
            moTa = 'Cân nặng của bạn đang ở mức lý tưởng!'
            goiY = 'Duy trì chế độ ăn cân bằng và tập luyện đều đặn 3-4 buổi/tuần.'
        } else if (bmi < 25) {
            phanLoai = 'Thừa cân nhẹ'
            mauSac = '#fbbf24'
            moTa = 'Bạn đang ở ngưỡng thừa cân nhẹ.'
            goiY = 'Giảm tinh bột, tăng rau xanh và protein. Cardio 30 phút/ngày kết hợp tập tạ.'
        } else if (bmi < 30) {
            phanLoai = 'Thừa cân'
            mauSac = '#f97316'
            moTa = 'Bạn đang thừa cân, cần điều chỉnh.'
            goiY = 'Tạo thâm hụt calo 300-500 kcal/ngày. Cardio đều đặn, hạn chế đồ ngọt và đồ chiên rán.'
        } else {
            phanLoai = 'Béo phì'
            mauSac = '#ef4444'
            moTa = 'Bạn đang ở mức béo phì, cần thay đổi ngay.'
            goiY = 'Tham khảo chuyên gia dinh dưỡng. Bắt đầu với đi bộ 30-45 phút/ngày, giảm dần calo nạp vào.'
        }
        ...
        // Tính TDEE cơ bản (BMR Mifflin-St Jeor)
        let bmr
        if (gioiTinh === 'nam') {
        ...
        const tdee = Math.round(bmr * 1.55) // mức hoạt động trung bình
    ...
    return (
        <div>
            {/* Form nhập */}
            <div className="card">
                <div className="card-title">⚖️ Tính chỉ số BMI</div>
                ...
                <div style={{ marginTop: '16px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        {[{ val: 'nam', label: '👨 Nam' }, { val: 'nu', label: '👩 Nữ' }].map(g => (
                            <div
                                key={g.val}
                                onClick={() => setGioiTinh(g.val)}
                                className={`goal-card ${gioiTinh === g.val ? 'active' : ''}`}
                                style={{ padding: '14px', flexDirection: 'row', justifyContent: 'center', gap: '8px' }}
                            >
                ...
                <button style={{ marginTop: '20px' }} onClick={tinhBMI}>
                    📊 Tính BMI
                </button>
            </div>
                ...
                <div className="card" style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ marginBottom: '24px' }}>
                        <div style={{
                            height: '12px',
                            borderRadius: '99px',
                            background: 'linear-gradient(to right, #60a5fa, #00d4a0, #fbbf24, #f97316, #ef4444)',
                            position: 'relative',
                            marginBottom: '8px'
                        }}>
                            <div style={{
                                ...
                                width: '20px',
                                height: '20px',
                                background: ketQua.mauSac,
                                borderRadius: '50%',
                                border: '3px solid #fff',
                                transform: 'translateX(-50%)',
                                boxShadow: `0 0 10px ${ketQua.mauSac}`
                            }} />
                        </div>
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--accent)', marginBottom: '8px' }}>💡 Gợi ý cho bạn</div>
            ...
            {/* Bảng phân loại BMI */}
            <div className="card">
                <div className="card-title">📖 Bảng phân loại BMI (Châu Á)</div>
                {[
