...
function TrangChu() {
    const {
        tuoi, canNang, chieuCao,
        mucTieu, soNgay, setSoNgay,
        setKetQua, setLoading, user
    } = useApp()

    const [chatContext, setChatContext] = useState('')

    const bmi = canNang && chieuCao ? canNang / Math.pow(chieuCao / 100, 2) : 0
    ...
    const taoLichTap = async () => {
        ...
        const prompt = `Bạn là huấn luyện viên thể hình. Trả lời CHỈ bằng JSON hợp lệ.
...
- Mục tiêu: ${mucTieu}
- Số ngày tập: ${soNgay}
${chatContext ? `\nThông tin bổ sung từ AI Coach:\n${chatContext}` : ''}

Không giải thích. Không markdown. Chỉ JSON.`

        try {
            const data = await callGemini(import.meta.env.VITE_GEMINI_KEY, {
    ...
    return (
        <div className="trangchu-container">
                    ...
                    <div className="card">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                            {options.map(opt => (
                                ...
                                >
                                    <strong style={{ fontSize: '22px', color: soNgay === opt ? 'var(--accent)' : 'var(--text)' }}>
                                        {opt.replace(' ngày', '')}
                                    </strong>
            ...
            <AICoach onContextUpdate={setChatContext} />
        </div>
    )
}
