...
function AppLayout() {
    const { user } = useAuth()
    const { profileLoaded, tuoi, canNang, chieuCao } = useApp()
    ...
    // Check onboarding status từ Firestore flag
    useEffect(() => {
        if (!user || !profileLoaded) return
        const check = async () => {
            // Check localStorage trước cho nhanh
    ...
    // Loading profile - chỉ chờ profileLoaded, không chờ onboardingDone
    if (!profileLoaded) {
        return (
            <div style={{
